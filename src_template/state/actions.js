...
export const handleSaleUpdate = async (
  ...
  const sale = await account.viewFunction(marketId, "get_sale", {
    nft_contract_token: stakingContractId + DELIMETER + token_id,
  });
  if (sale) {
    await account.functionCall(
      marketId,
...
export const handleStake = async (account, token_id, newSaleConditions) => {
  await account.functionCall(
    nftContractId,
    "nft_approve",
    {
