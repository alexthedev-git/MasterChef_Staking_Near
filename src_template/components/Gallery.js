...
import {
  getMarketStoragePaid,
  loadItems,
  getSupplyStakingInformations,
  getClaimAmount,
...
export const Gallery = ({
  ...
  const { tokenInfo, claimAmount, tokens, allTokens, marketStoragePaid } =
    views;
  ...
  useEffect(() => {
    if (!loading) {
      ...
      dispatch(getSupplyStakingInformations(account));
      dispatch(getStakingInformation(account));
      dispatch(getClaimAmount(account));
  ...
  const token = tokens.find(({ token_id }) => tokenId === token_id);
  if (token) {
  ...
  return (
    <>
      ...
      {tab === 1 && (
        <>
          <h1>Claim Amount: {claimAmount}</h1>
          {!tokens.length && (
