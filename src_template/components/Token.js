...
export const Token = ({
  ...
  account,
  token,
  token: { token_id, nft_contract_id, metadata, owner_id, conditions, bids },
}) => {
  ...
  const handleShare = async (e) => {
    ...
    const actions = JSON.stringify({
      ...
      redirect: encodeURIComponent(
        `${window.location.origin}${window.location.pathname}?t=${token_id}`
      ),
      encodeUrl: true,
    });
    ...
    if (!response || !response.encodedUrl) {
      ...
      return alert(
        "Something went wrong trying to share this url, please try sharing from the address bar or use your browsers share feature"
      );
    }
  ...
  return (
    <div className="token">
      <div onClick={() => history.pushState({}, "", window.location.pathname)}>
        ...
        <img crossOrigin="anonymous" src={metadata.media} />
      </div>
      <div className="token-detail">
        <div>
          <a href={explorerUrl + "/accounts/" + owner_id}>{owner_id}</a>
        </div>
        <br />
        <div>
        ...
        {conditions?.near.length && (
          <>
            ...
            {Object.entries(conditions).map(([ft_token_id, price]) => (
        ...
        {bids?.near && (
          <>
            ...
            {Object.entries(bids).map(
              ([ft_token_id, { owner_id: bid_owner_id, price }]) => (
